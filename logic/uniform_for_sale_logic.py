...
class UniformForSaleService:
    @staticmethod
    def create_uniform(db: Session, uniform_data: CreateUniformForSaleDTO) -> UniformForSaleDTO:
    ...
    def get_uniform(db: Session, uniform_id: int) -> UniformForSaleDTO | None:
        uniform = db.query(UniformForSale).filter(UniformForSale.id == uniform_id).first()
    ...
    @staticmethod
    def update_uniform(db: Session, uniform_id: int, uniform_data: CreateUniformForSaleDTO) -> UniformForSaleDTO | None:
        uniform = db.query(UniformForSale).filter(UniformForSale.id == uniform_id).first()
        if not uniform:
            return None
