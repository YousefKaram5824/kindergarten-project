...
from DTOs.monthly_finance_overall_dto import MonthlyFinanceOverallDTO, CreateMonthlyFinanceOverallDTO
from mapper import map_to_dto, map_to_model, update_model_from_dto

class MonthlyFinanceOverallService:
    @staticmethod
    def create_monthly(db: Session, data: CreateMonthlyFinanceOverallDTO) -> MonthlyFinanceOverallDTO:
    ...
    def get_monthly(db: Session, month_id: int) -> MonthlyFinanceOverallDTO | None:
        month = db.query(MonthlyFinanceOverall).filter(MonthlyFinanceOverall.id == month_id).first()
    ...
    @staticmethod
    def update_monthly(db: Session, month_id: int, data: CreateMonthlyFinanceOverallDTO) -> MonthlyFinanceOverallDTO | None:
        month = db.query(MonthlyFinanceOverall).filter(MonthlyFinanceOverall.id == month_id).first()
        if not month:
            return None
