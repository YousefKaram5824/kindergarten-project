...
from DTOs.monthly_finance_full_day_dto import MonthlyFinanceFullDayDTO, CreateMonthlyFinanceFullDayDTO
from mapper import map_to_dto, map_to_model, update_model_from_dto

class MonthlyFinanceFullDayService:
    @staticmethod
    def create_monthly(db: Session, data: CreateMonthlyFinanceFullDayDTO) -> MonthlyFinanceFullDayDTO:
    ...
    def get_monthly(db: Session, month_id: int) -> MonthlyFinanceFullDayDTO | None:
        month = db.query(MonthlyFinanceFullDay).filter(MonthlyFinanceFullDay.id == month_id).first()
    ...
    @staticmethod
    def update_monthly(db: Session, month_id: int, data: CreateMonthlyFinanceFullDayDTO) -> MonthlyFinanceFullDayDTO | None:
        month = db.query(MonthlyFinanceFullDay).filter(MonthlyFinanceFullDay.id == month_id).first()
        if not month:
            return None
