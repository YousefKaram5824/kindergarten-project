...
from DTOs.monthly_finance_individual_dto import MonthlyFinanceIndividualDTO, CreateMonthlyFinanceIndividualDTO
from mapper import map_to_dto, map_to_model, update_model_from_dto

class MonthlyFinanceIndividualService:
    @staticmethod
    def create_monthly(db: Session, data: CreateMonthlyFinanceIndividualDTO) -> MonthlyFinanceIndividualDTO:
    ...
    def get_monthly(db: Session, month_id: int) -> MonthlyFinanceIndividualDTO | None:
        month = db.query(MonthlyFinanceIndividual).filter(MonthlyFinanceIndividual.id == month_id).first()
    ...
    @staticmethod
    def update_monthly(db: Session, month_id: int, data: CreateMonthlyFinanceIndividualDTO) -> MonthlyFinanceIndividualDTO | None:
        month = db.query(MonthlyFinanceIndividual).filter(MonthlyFinanceIndividual.id == month_id).first()
        if not month:
            return None
