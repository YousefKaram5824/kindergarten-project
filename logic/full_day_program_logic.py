...
class FullDayProgramService:
    @staticmethod
    def create_program(db: Session, program_data: CreateFullDayProgramDTO) -> FullDayProgramDTO:
    ...
    def get_program(db: Session, program_id: int) -> FullDayProgramDTO | None:
        program = db.query(FullDayProgram).filter(FullDayProgram.id == program_id).first()
    ...
    @staticmethod
    def update_program(db: Session, program_id: int, program_data: CreateFullDayProgramDTO) -> FullDayProgramDTO | None:
        program = db.query(FullDayProgram).filter(FullDayProgram.id == program_id).first()
        if not program:
            return None
