...
def main(page: ft.Page):
        ...
        forgot_password_page = ft.Column(
            [
                ft.Row([
                    ...
                    ft.Text("إعادة تعيين كلمة المرور", size=24, weight=ft.FontWeight.BOLD, expand=True, text_align=ft.TextAlign.CENTER)
    ...
    def handle_password_reset(e):
        username = reset_username_field.value.strip() if reset_username_field.value else ""
        ...
        success, message = auth_manager.reset_password(
            username, new_password, admin_username_field.value, admin_password_field.value
        )
    ...
    def show_create_account_page():
        ...
        create_account_page = ft.Column(
            [
                ft.Row([
                    create_back_button,
                    ft.Text("إنشاء حساب جديد", size=24, weight=ft.FontWeight.BOLD, expand=True, text_align=ft.TextAlign.CENTER)
                ], alignment=ft.MainAxisAlignment.START),
                ft.Divider(),
                ft.Text("التحقق من هوية المدير", size=18, weight=ft.FontWeight.BOLD),
                admin_username_field,
    ...
    def handle_create_account(e):
        ...
        password = new_user_password_field.value.strip() if new_user_password_field.value else ""
        confirm_password = confirm_new_password_field.value.strip() if confirm_new_password_field.value else ""
    ...
    login_page = ft.Column(
        [
            ...
            login_button,
            login_error_text,
            ft.Row([forgot_password_link, create_account_link], 
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=20)
        ],
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    ...
    def show_main_system():
        ...
        refresh_students_btn = ft.ElevatedButton(
            "تحديث القائمة",
            on_click=lambda e: update_student_table()
        )
