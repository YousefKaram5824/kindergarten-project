...
def create_reports_tab(page: ft.Page, financial_records, inventory_items):
    ...
    report_content = ft.TextField(
        multiline=True, read_only=True, expand=True, border=ft.InputBorder.NONE, text_align=ft.TextAlign.RIGHT
    )

    def generate_report(e):
        with db_session() as db:
            students_data = ChildService.get_all_children(db)
            ...
            الطلاب:
            {chr(10).join([f"- {s.name} (العمر: {s.age or 'غير محدد'})" for s in students_data])}
    ...
    return ft.Column(
        [
            ft.Text("التقارير", size=24, weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.RIGHT),
            ft.Divider(),
