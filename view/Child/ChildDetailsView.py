...
class ChildDetailsView:
    def __init__(self, page: ft.Page, child_id: int, update_callback=None, is_edit=False):
        self.page = page
    ...
    def create_layout(self):
        ...
        self.full_day_section = ft.Container(
            content=ft.Column([
                ft.Text("بيانات البرنامج الكامل", size=20, weight=ft.FontWeight.BOLD, color=ft.Colors.GREEN_700),
                ft.Divider(),
                ft.Row([
                ...
                ft.Container(height=20),
                ft.Text("الملفات الخاصة بالبرنامج الكامل:", size=16, weight=ft.FontWeight.BOLD),
                ft.Row([
                    self.training_plan_card
                ], alignment=ft.MainAxisAlignment.CENTER)
            ]),
            padding=20,
        ...
        self.sessions_section = ft.Container(
            content=ft.Column([
                ft.Text("بيانات الجلسات الفردية", size=20, weight=ft.FontWeight.BOLD, color=ft.Colors.ORANGE_700),
                ft.Divider(),
                ft.Row([
        ...
        files_section = ft.Container(
            content=ft.Column([
                ft.Text("الملفات والمستندات", size=20, weight=ft.FontWeight.BOLD, color=ft.Colors.PURPLE_700),
                ft.Divider(),
                ft.Row([
                ...
                ft.Container(height=20),
                ft.Row([
                    self.tests_applied_card,
                    self.monthly_report_card
                ], alignment=ft.MainAxisAlignment.CENTER, wrap=True)
            ]),
            padding=20,
        ...
        # Remove None values
        action_buttons.controls = [btn for btn in action_buttons.controls if btn is not None]
    ...
    def load_existing_data(self):
        with db_session() as db:
            ...
            self.type_dropdown.value = child.child_type.name if child.child_type != ChildTypeEnum.NONE else None
            
            if child.child_type == ChildTypeEnum.FULL_DAY:
                program = FullDayProgramService.get_program_by_child_id(db, self.child_id)
                if program:
                ...
                session = IndividualSessionService.get_session_by_child_id(db, self.child_id)
                if session:
    ...
    def load_full_day_data(self, program):
        ...
        self.monthly_fee_field.value = str(program.monthly_fee) if program.monthly_fee else ""
    ...
    def load_sessions_data(self, session):
        ...
        self.session_fee_field.value = str(session.session_fee) if session.session_fee else ""
        self.monthly_sessions_count_field.value = str(session.monthly_sessions_count) if session.monthly_sessions_count else ""
        self.attended_sessions_count_field.value = str(session.attended_sessions_count) if session.attended_sessions_count else ""
        ...
        if file_path and os.path.exists(file_path):
            ...
            if file_ext in ['.jpg', '.jpeg', '.png']:
                ...
                preview_content = ft.Column([
                    ...
                    ft.Text(filename, size=10, text_align=ft.TextAlign.CENTER, max_lines=2)
                ], horizontal_alignment=ft.CrossAxisAlignment.CENTER)
            else:
                preview_content = ft.Column([
                    ft.Icon(ft.Icons.DESCRIPTION, size=60, color=ft.Colors.BLUE_400),
                    ft.Text(filename, size=10, text_align=ft.TextAlign.CENTER, max_lines=2)
                ], horizontal_alignment=ft.CrossAxisAlignment.CENTER)
            ...
            for control in actions_row.controls:
                if control.icon in [ft.Icons.VISIBILITY, ft.Icons.DOWNLOAD, ft.Icons.DELETE]:
    ...
    def handle_file_picker_result(self, e: ft.FilePickerResultEvent):
        if e.files and self.current_file_type:
            ...
            # Update preview
            self.update_file_preview(self.current_file_type, file_path)
            
            # Show success message
            self.show_snackbar(f"تم رفع {self.get_file_type_name(self.current_file_type)} بنجاح", ft.Colors.GREEN)
    ...
    def delete_file(self, file_type):
        def confirm_delete(e):
            ...
            self.show_snackbar(f"تم حذف {self.get_file_type_name(file_type)}", ft.Colors.ORANGE)
    ...
    def save_full_day_program(self, db):
        existing_program = FullDayProgramService.get_program_by_child_id(db, self.child_id)
        
        program_data = {
            "diagnosis": self.diagnosis_field.value or None,
            "monthly_fee": float(self.monthly_fee_field.value) if self.monthly_fee_field.value else None,
            "bus_fee": float(self.bus_fee_field.value) if self.bus_fee_field.value else None,
    ...
    def save_individual_session(self, db):
        existing_session = IndividualSessionService.get_session_by_child_id(db, self.child_id)
        
        session_data = {
            "diagnosis": self.diagnosis_field.value or None,
            "session_fee": float(self.session_fee_field.value) if self.session_fee_field.value else None,
            "monthly_sessions_count": int(self.monthly_sessions_count_field.value) if self.monthly_sessions_count_field.value else None,
            "attended_sessions_count": int(self.attended_sessions_count_field.value) if self.attended_sessions_count_field.value else None,
...
def create_child_details_view(page: ft.Page, child_id: int, update_callback=None, is_edit=False):
    """
...
def open_child_edit_view(page: ft.Page, child_id: int, update_callback=None):
    ...
    page.clean()
    child_view = create_child_details_view(page, child_id, update_callback, is_edit=True)
    page.add(child_view)
    page.update()
...
def open_child_view_only(page: ft.Page, child_id: int, update_callback=None):
    ...
    page.clean()
    child_view = create_child_details_view(page, child_id, update_callback, is_edit=False)
    page.add(child_view)
    page.update()
