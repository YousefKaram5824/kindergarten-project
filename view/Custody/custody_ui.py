...
def create_custody_tab(page: ft.Page, db: Session):
        ...
        for i, t in enumerate(tools):
            ...
            data_table.rows.append(
                ft.DataRow(
                    color=row_color,
                    cells=[
                        ft.DataCell(ft.Container(ft.Text(str(t.id),
                                                         weight=ft.FontWeight.BOLD,
                                                         color=ft.Colors.BLUE_800), padding=8)),
                        ft.DataCell(ft.Container(ft.Text(t.tool_name,
                                                         weight=ft.FontWeight.W_500,
                                                         size=14), padding=8)),
                        ft.DataCell(ft.Container(
                            ft.Chip(label=ft.Text(t.department or "غير محدد", size=12),
                                    bgcolor=ft.Colors.BLUE_100,
                                    color=ft.Colors.BLUE_800)
                            if t.department else ft.Text("غير محدد", color=ft.Colors.GREY_600),
                            padding=8)),
                        ft.DataCell(ft.Container(ft.Text(
                            str(t.purchase_date) if t.purchase_date else "غير محدد",
                            color=ft.Colors.GREY_700), padding=8)),
                        ft.DataCell(ft.Container(ft.Text(
                            t.notes[:30] + "..." if t.notes and len(t.notes) > 30 else t.notes or "لا توجد ملاحظات",
                            color=ft.Colors.GREY_600, size=12), padding=8)),
                        ft.DataCell(ft.Container(
                            ft.Container(
                                ft.Image(src=t.tool_image, width=40, height=40,
                                         fit=ft.ImageFit.COVER, border_radius=8,)
                                if t.tool_image else ft.Icon(ft.Icons.BUILD_CIRCLE,
    ...
    def confirm_delete(tool_id: int, tool_name: str, e=None):
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[
                ft.TextButton("إلغاء", on_click=lambda e: (setattr(dlg, "open", False), page.update()),
                              style=ft.ButtonStyle(color=ft.Colors.GREY_600)),
                ft.ElevatedButton("حذف", on_click=delete_confirmed,
                                  bgcolor=ft.Colors.RED_600, color=ft.Colors.WHITE, icon=ft.Icons.DELETE),
            ],
    ...
    def open_tool_form(title: str, tool_id: int | None):
        ...
        image_field = ft.TextField(label="رابط الصورة",
        ...
        def submit_form(e):
            ...
            try:
                if tool_id:
                    edit_tool(tool_id, UpdateTrainingToolDTO(
        ...
        dlg = ft.AlertDialog(
            modal=True,
            title=ft.Row([
                ft.Icon(ft.Icons.ADD_CIRCLE if not tool_id else ft.Icons.EDIT, color=ft.Colors.BLUE_600),
                ft.Text(title, weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_800),
            ]),
            content=ft.Container(ft.Column([name_field, dept_field, purchase_field,
                                            notes_field, image_field],
                                           tight=True, spacing=10),
                                 width=400, padding=10),
            actions=[
                ft.TextButton("إلغاء", on_click=lambda e: (setattr(dlg, "open", False), page.update()),
                              style=ft.ButtonStyle(color=ft.Colors.GREY_600)),
                ft.ElevatedButton("حفظ", on_click=submit_form,
        ...
        ft.Row([
            ft.Container(ft.Row([
                ...
                ft.TextField(
                    ...
                    on_change=lambda e: load_tools(e.control.value) if not e.control.value else None,
    ...
    data_table = ft.DataTable(
        columns=[
            ...
            ft.DataColumn(ft.Text("اسم الأداة", weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_800)),
            ft.DataColumn(ft.Text("القسم", weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_800)),
            ft.DataColumn(ft.Text("تاريخ الشراء", weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_800)),
            ft.DataColumn(ft.Text("ملاحظات", weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_800)),
            ft.DataColumn(ft.Text("صورة", weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_800)),
            ft.DataColumn(ft.Text("إجراءات", weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_800)),
        ],
        rows=[],
        border=ft.border.all(1, ft.Colors.GREY_300),
        border_radius=10,
    ...
    table_container = ft.Container(
        ...
        padding=10,
        shadow=ft.BoxShadow(
            spread_radius=1, blur_radius=15, color=ft.Colors.GREY_300, offset=ft.Offset(0, 2),
        ),
    )

    content = ft.Container(
        ft.Column([header, table_container], spacing=0),
        padding=20,
        bgcolor=ft.Colors.GREY_100,
        expand=True,
    )
