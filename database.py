...
class KindergartenDatabase:
    ...
    # Parent operations
    def add_parent(
        self, student_id: int, name: str, job: str, relationship: str
    ) -> bool:
    ...
    def add_financial_record(
        self, student_id: int, monthly_fee: float, bus_fee: float, month_year: str
    ) -> bool:
    ...
    def add_inventory_item(
        self, item_name: str, quantity: int, purchase_price: float, description: str
    ) -> bool:
        """Add inventory item"""
        try:
            if not self.connect():
