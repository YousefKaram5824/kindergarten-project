...
def show_forgot_password_dialog(page: ft.Page):
    ...
    dialog = ft.AlertDialog(
        modal=True,
        title=ft.Text("إعادة تعيين كلمة المرور"),
        content=ft.Column(
            [
...
def show_create_account_dialog(page: ft.Page):
    ...
    def handle_create(e):
        ...
        admin_auth, admin_msg = auth_manager.verify_admin(admin_username, admin_password)
        if not admin_auth:
...
def show_login_page(page: ft.Page):
    ...
    forgot_password_link = ft.TextButton(
        "نسيت كلمة المرور؟",
        on_click=lambda e: show_forgot_password_dialog(page)
    )
    
    create_account_link = ft.TextButton(
        "إنشاء حساب جديد",
        on_click=lambda e: show_create_account_dialog(page)
    )
    ...
    login_page = ft.Column(
        [
            ...
            password_field,
            login_button,
            ft.Row([forgot_password_link, create_account_link], alignment=ft.MainAxisAlignment.CENTER),
            login_error_text
        ],
